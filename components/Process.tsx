...
import type { ProcessContent } from "@/lib/page-content";
    ...
    return (
        <section id="process" className={`${getSectionSpacing()} bg-gray-50 dark:bg-gray-800/50 relative overflow-hidden ${getSectionSeparator()}`}>
            ...
            <div ref={ref} className={`${getSectionContainer()} ${getSectionPadding()} relative z-10`}>
                <SectionHeader
                    badge={content?.badge || "كيف أعمل"}
                    title={content?.title || "رحلة"}
                    highlight={content?.highlight || "الإبداع"}
                    description={content?.description || "عملية منظمة تضمن جودة استثنائية في كل مشروع"}
                    isInView={isInView}
                />
                        ...
                        {steps.map((step, index) => (
                            ...
                            >
                                <m.div
                                    ...
                                    className={`w-20 h-20 mx-auto mb-6 rounded-2xl bg-gradient-to-br ${COLORS[index % COLORS.length]} flex items-center justify-center text-white shadow-lg relative z-10`}
                                >
                                ...
                                <div className="text-center">
                                    <h3 className="text-xl font-bold text-gray-800 dark:text-white mb-3 break-words">{step.title}</h3>
                        ...
                        >
                            <div className="relative flex-shrink-0">
                                <m.div
                                    whileHover={{ scale: 1.05 }}
                                    transition={{ duration: 0.2, ease: "easeOut" }}
                                    className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${COLORS[index % COLORS.length]} flex items-center justify-center text-white shadow-lg`}
                                >
                                    {ICONS[index % ICONS.length]}
                                </m.div>
                            ...
                            <div className="flex-1 min-w-0">
                                <h3 className="text-xl font-bold text-gray-800 dark:text-white mb-2 break-words">{step.title}</h3>
                                <p className="text-sm text-gray-600 dark:text-gray-300 leading-relaxed break-words">{step.description}</p>
                            </div>
                        </m.div>
                    ))}
